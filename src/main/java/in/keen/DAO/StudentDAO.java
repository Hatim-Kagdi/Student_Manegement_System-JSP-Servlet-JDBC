...
public class StudentDAO {
	...
	// Search Students
	public List<Student> searchStudent(String query, int currentPage, int pageSize) {
		List<Student> list = new ArrayList<>();
		int offset = (currentPage - 1) * pageSize;
		String sql = "SELECT * FROM students WHERE (students_name LIKE ? OR students_id LIKE ?) and is_deleted = false LIMIT ? OFFSET ?";
	...
	public int getSearchedStudentCount(String searchQuery) {
		String query = "SELECT COUNT(*) FROM students WHERE (students_id LIKE ? OR students_name LIKE ?) AND is_deleted = false";
	...
	public boolean updateStudentField(int studentId, String fieldName, String newValue) {
		...
		try {
			...
			String userQuery = "UPDATE users SET " + userColumn
					+ " = ? WHERE user_id = (SELECT user_id FROM students WHERE students_id = ?)";
	...
	public List<Student> getStudentsByPage(int currentPage, int pageSize) {
		List<Student> list = new ArrayList<>();
		int offset = (currentPage - 1) * pageSize;
		String query = "SELECT * FROM students WHERE is_deleted = false LIMIT ? OFFSET ?";
		try (Connection con = DBconnection.getConnection(); PreparedStatement ps = con.prepareStatement(query)) {
	...
	public int getTotalStudentsCount() {
		String query = "SELECT COUNT(*) FROM students WHERE is_deleted = false";
		try (Connection con = DBconnection.getConnection(); PreparedStatement ps = con.prepareStatement(query)) {
			ResultSet rs = ps.executeQuery();
			if (rs.next())
				return rs.getInt(1);
